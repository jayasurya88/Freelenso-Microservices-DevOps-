...
def approve_milestone(request, project_id, milestone_id):
    ...
    try:
        with transaction.atomic():
            # Get client's wallet
            client_wallet = Wallet.objects.get(user=project.client)
            
            # Get freelancer's wallet
            freelancer_wallet, created = Wallet.objects.get_or_create(user=project.assigned_freelancer)
            
            # Update milestone status
            milestone.status = 'approved'
            milestone.save()
            
            # Transfer funds from escrow to freelancer's wallet
            ...
            if project.completed_milestones == project.total_milestones:
                ...
                create_notification(
                    recipient=project.assigned_freelancer,
                    notification_type='milestone',
                    message=f'Milestone "{milestone.title}" has been approved in project "{project.title}" and payment has been released to your wallet.',
                    sender=request.user.userprofile,
                    project=project
                )
                
                messages.success(request, 'Milestone approved successfully! Payment has been released to the freelancer.')
...
def wallet_escrow_records(request):
    ...
    if user_profile.is_client:
        ...
        # For freelancers, show all funded milestones in their projects
        ...
        for project in freelancer_projects:
            ...
            milestones = ProjectMilestone.objects.filter(
                project=project,
                status__in=['funded', 'completed']  # Show both funded and completed milestones
            )
            
            for milestone in milestones:
                # Get the corresponding escrow transaction
    ...
    active_escrows.sort(key=lambda x: x['date'], reverse=True)
